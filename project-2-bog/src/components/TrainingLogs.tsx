...
const TrainingLogs = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh logs after successful operation
      await fetchLogs();
      
      // Reset form state
      setNewLog({ date: '', title: '', animal: '', hours: '', description: '' });
      setEditingLog(null);
      setIsFormOpen(false);
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 relative">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-gray-700">Training logs</h2>
        <button 
          onClick={() => {
            setEditingLog(null);
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 w-full max-w-lg mx-4">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium mb-1 text-gray-700">Total hours trained</label>
              <input
                type="text"
                name="hours"
                className="w-full border border-gray-700 rounded p-2 text-gray-700"
